...
def test_typed_websocket_strict_drift_frame_is_skipped(page_on, serving_mode):
    if serving_mode == "static":
        pytest.skip("WebSocket endpoint exists only in prod mode (asgi-mount)")
    ...
    page.locator("[data-testid='typed-extra-btn']").click()
    expect(page.locator("[data-testid='typed-error']")).not_to_have_text("", timeout=30_000)
    expect(page.locator("[data-testid='typed-message-item']")).to_have_count(1, timeout=30_000)

    page.locator("[data-testid='typed-roundtrip-btn']").click()
    expect(page.locator("[data-testid='typed-message-item']")).to_have_count(2, timeout=30_000)
