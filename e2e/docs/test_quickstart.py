...
def test_middle_page_shows_both_links(docs_page_on, assert_no_console_errors):
    ...
    expect(page.locator(".docs-pager-next a")).to_have_text("UI Primitives")
...
def test_next_navigation_updates_sidebar_active(docs_page_on, assert_no_console_errors):
    ...
    expect(page).to_have_url(re.compile(r"/documents/basic/ui-primitives"))
    expect(page.get_by_role("heading", name="UI Primitives")).to_be_visible()
    expect(page.locator(".docs-sidebar a[aria-current='page']")).to_have_text("UI Primitives")
