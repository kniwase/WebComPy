...
Progressive Web App support — installability and offline capability — provided as declarative build configuration with zero user JavaScript. The framework generates the Web App Manifest and a framework-owned vanilla-JS Service Worker at build time (the Angular ngsw model): precache manifests are enumerated from build output with content-hashed assets, runtime caching follows explicit same-origin pattern rules, worker updates activate immediately and clean previous caches, offline navigations serve a fallback page, and registration is injected at the correct base-URL scope. PWA is disabled by default and stays out of the dev server unless explicitly enabled.
