# UI Overlay Components Specification

## Purpose

First-party overlay components — Modal, Drawer, Dropdown, and Toast — as headless/themed pairs built on the Teleport, Transition, and UI primitives foundations. The headless layer owns behavior, ARIA, keyboard interaction, focus management, and trigger anchoring; the themed layer composes it with token-based defaults delivered through the primitives stylesheet. Overlay state (open/close, toast queue) follows the reactive contracts, overlay content renders at the document root through Teleport with Transition-driven open/close animation, and dropdown menus anchor to their trigger by default.
