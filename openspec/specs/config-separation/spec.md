...
`WebComPyServerConfig` SHALL be a lightweight dataclass with `port: int = 8080` and `dev: bool = False`. It SHALL be importable from `webcompy_cli.config` (with legacy shim at `webcompy.cli.config`). It SHALL be a nested field of `WebComPyBuildConfig` via the `server` attribute.
...
When using WebComPy as a library in PyScript (importing via CDN or direct wheel), a developer SHALL only need to import from `webcompy.app`. No import from `webcompy_cli.config` or `webcompy_server` SHALL be required for library usage. The structural separation between `webcompy` (core) and `webcompy_cli` (CLI) packages makes this enforcement natural — `webcompy_cli` is simply not present in the browser wheel.
...
### ADDED: Config separation is structurally enforced by separate packages

The separation between browser and server configuration SHALL be structurally enforced by the package topology. `WebComPyAppConfig` SHALL live in `webcompy.app` (core package, included in browser wheels). `WebComPyBuildConfig` and `WebComPyServerConfig` SHALL live in `webcompy_cli.config` (CLI package, never in browser wheels). This SHALL eliminate the need for `_BROWSER_ONLY_EXCLUDE` as a mechanism to keep server-only code out of browser wheels — the physical package boundary provides natural exclusion.

#### Scenario: Browser wheel contains only webcompy.app config
- **WHEN** a browser wheel is built
- **THEN** no files from `webcompy_cli` SHALL be present
- **AND** `WebComPyAppConfig` SHALL be available from `webcompy.app`
- **AND** `WebComPyBuildConfig` and `WebComPyServerConfig` SHALL NOT be available in the browser environment

#### Scenario: Legacy shim supports old import path during transition
- **WHEN** a developer writes `from webcompy.cli.config import WebComPyBuildConfig`
- **THEN** the import SHALL succeed via the legacy shim
- **AND** the object SHALL be the same as `webcompy_cli.config.WebComPyBuildConfig`