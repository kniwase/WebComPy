...
Governs docstring coverage, content, and consistency for the public interfaces of the `webcompy`, `webcompy-server`, `webcompy-cli`, and `webcompy-testing` packages, and provides a machine-checkable guardrail so coverage cannot regress, docstrings or comments never leak OpenSpec project-management artifacts, and docstrings stay synchronized with implementation within the same PR.
