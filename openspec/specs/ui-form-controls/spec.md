# UI Form Controls Specification

## Purpose

First-party form controls — Input, Textarea, Select, Checkbox, Switch, Radio/RadioGroup, and the FormField wrapper — as headless/themed pairs built on the UI primitives foundation. Controls render native form elements through Light DOM custom element wrappers and bind either to a forms-module `Field` (via the framework `:bind` mechanism) or to raw value/change props. FormField composes label, control, and error region with correct ARIA wiring, deriving hydration-stable association ids from its transfer id and confining their DI provision to its own slot render pass. Validation state is exposed through the `data-state` vocabulary, and themed defaults live in the primitives stylesheet.
