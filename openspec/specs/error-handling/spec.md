...
The framework provides a structured error-handling pipeline so that a failure deep inside the component tree does not take down the whole application. An `ErrorBoundary` element catches descendant setup/render errors and swaps in a fallback, giving the user a chance to reset and retry. `on_error_captured` hooks let components intercept descendant errors before a boundary engages. Event-handler errors route through the same propagation walk but engage a boundary only when it opts in. During SSR, contained errors render fallbacks and the rest of the page continues; during SSG, any boundary error fails the build so broken pages never ship.
