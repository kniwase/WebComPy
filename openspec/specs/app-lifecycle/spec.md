...
- **AND** the summary SHALL show elapsed time between consecutive phases (`pyscript_ready → imports_done`, `imports_done → init_done`, `init_done → run_start`, `run_start → run_done`, `run_done → loading_off`) plus a total
