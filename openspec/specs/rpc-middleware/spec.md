...
Stackable processors around the HTTP JSON-RPC transports that let applications and plugins observe and transform typed procedure arguments, inject transport headers (auth tokens, tracing), short-circuit calls with validated synthetic results (browser-side mocks), and intercept streaming operations — without exposing the JSON-RPC wire format to middleware authors.
