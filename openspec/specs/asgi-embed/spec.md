...
Enable teams to embed a WebComPy serving app into an existing host ASGI application (e.g., FastAPI) via standard mounting. Embedded deployments get SSR pages, framework asset endpoints, and self-site fetch reaching the host's own routes — the APIs an embedded admin UI exists to consume.
