...
- **AND** if the `ClientOnly` element is removed from the DOM before the async render completes, `_remove_element()` SHALL cancel the stored `self._render_task` and dispose any partially rendered child nodes via `_remove_element()` on each successfully rendered child
