# UI Disclosure & Feedback Components Specification

## Purpose

First-party disclosure and feedback components — Tabs, Collapse, Accordion, Alert, Progress, Badge, Skeleton, and Card — as headless/themed pairs built on the UI primitives foundation. The headless layer owns behavior and ARIA (tablist keyboard contract, animated disclosure through Transition, announcement roles, progressbar state), and the themed layer composes it with token-based defaults delivered through the primitives stylesheet. Panels and disclosure content follow the reactive contracts with per-instance hydration-stable DOM ids, and the family is re-exported at the `webcompy.ui` top level.
