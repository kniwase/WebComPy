...
- **WHEN** a developer creates `count = Signal(0)` and `effect(lambda: print(count.value))`
...
- **WHEN** a developer creates `flag = Signal(True)`, `a = Signal("A")`, `b = Signal("B")`, and an effect that reads `a.value` when `flag.value` is True and `b.value` when `flag.value` is False
