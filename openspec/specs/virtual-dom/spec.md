...
### ADDED: VirtualDOMNode shall support comment nodes
...
### ADDED: ServerDOMPort.render_html() shall serialize comment nodes
...
### ADDED: Comment data shall not contribute to element text aggregation
...
#### Scenario: Class matching is case-sensitive while tag matching is not

- **WHEN** the document contains `<span class="Foo">` and `query_selector(".Foo")` is called
- **THEN** the returned node SHALL be that span
- **WHEN** `query_selector(".foo")` is called against the same document
- **THEN** `None` SHALL be returned
- **WHEN** `query_selector("SECTION.Foo")` is called against a document containing `<section class="Foo">`
- **THEN** the section SHALL match because type selectors are ASCII case-insensitive
