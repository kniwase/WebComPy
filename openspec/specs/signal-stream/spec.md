...
Provide bidirectional conversion between occurrence-based event streams and WebComPy's cell-based reactive primitives. `to_signal` bridges an async or plain iterable source into a `Signal` with a mandatory initial value; `to_reactive_list` accumulates every occurrence into a `ReactiveList` with optional drop-oldest capping; `to_async_iter` exposes a `Signal`'s updates as an async iterator. All bridges carry AsyncResult-style error and completion signals, integrate with component lifecycle cleanup, and produce derived client-side views that never participate in hydration transfer.
