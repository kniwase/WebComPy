...
#### Scenario: Initially-empty block keeps its markers and receives deferred content

- **WHEN** a teleported child renders no nodes at emission time (e.g. a repeated list that is initially empty) and a scheduled task adds items during the settle that follows the registry drain
- **THEN** the SSR HTML SHALL still contain the start/end marker pair for that teleport
- **AND** the deferred items SHALL be serialized inside the delimited block
- **AND** during hydration the Teleport SHALL claim the block and reproduce its content exactly once

### Requirement: Teleport block markers shall delimit emitted blocks for deterministic hydration consumption

Each server-emitted Teleport block SHALL be delimited by a start marker comment and an end marker comment carrying a per-document ordinal and the URL-encoded `to` selector (`wc-teleport-block:<n>:<selector>` / `wc-teleport-block-end:<n>`), such that: markers survive HTML parsing as distinct comment nodes; ordinals are assigned in registry order, which equals the application's document-order traversal of Teleports and is identical in the server render pass and the client hydration pass; a hydration Teleport can locate its own block by scanning its resolved target for the first unclaimed start marker that matches its selector sequence. A Teleport SHALL be enqueued for emission at most once per server render context even when its element tree renders multiple times (e.g. a router-driven render pass followed by the main document render pass), so each Teleport produces exactly one block. Markers SHALL carry no styling or layout impact.
