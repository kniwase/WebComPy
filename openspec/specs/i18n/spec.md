...
Reactive internationalization core: an `I18nManager` living in the application DI scope holds a locale `Signal`, nested message catalogs, and a fallback locale, and `use_i18n()` exposes the locale signal, a `t` translation function, and a locale-switching controller. Translations track the locale signal through normal reactive dependency tracking, so templates re-render on locale switch without subscription APIs. Catalog resolution uses dot-path keys with `{param}` interpolation and CLDR plural categories (built-in minimal table plus an opt-in Babel adapter), and locale persistence is cookie-only with identical, deterministic normalization on server and browser so first render and hydration always agree.
