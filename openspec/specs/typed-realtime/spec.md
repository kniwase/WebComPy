...
Realtime applications exchange structured messages (dataclasses), not raw text. WebComPy already owns the serialization stack — schema-driven `from_json`, framework-neutral `encode_with_meta` + body wire mode, and the json-rpc allowlist pattern for safe type-tag decoding — but none of it was wired into the realtime composables. `typed-realtime` layers typed message send/receive onto `use_websocket`: the `message_type` contract, the typed-response body wire envelope, strict-by-default schema reconstruction with skip-on-error `.last_error` surfacing, and an app-scoped realtime type allowlist. SSR and hydration behavior are unchanged from `websocket-composable`.
