...
The ResourcePort provides an async, injectable abstraction for reading application resource files (HTML, CSS, Markdown, images, fonts, etc.) in both server (SSR/SSG build) and browser (CSR/initial hydration) environments. Resources are identified by package-relative POSIX paths. The browser port prefers content embedded in the hydration payload and falls back to fetching from a server endpoint.
...
1. Validate the path format (reject empty path, absolute path, `..` segments).
2. Read `RESOURCE_DATA_KEY` from the DI scope (`InjectKey[dict[str, str]]`, base64-encoded bytes keyed by package-relative path). If the path is present, decode and return the content (text or bytes based on the called method).
3. Otherwise, fetch `GET {base_url}_webcompy-resource/{path}` via the existing `FetchPort` (URL prefix comes from `WebComPyAppConfig.base_url`). On HTTP success, return the response content (text or bytes based on the called method, using `response.content` for bytes).
4. On HTTP non-2xx, missing payload entry, or fetch error, raise `ResourceNotFoundError` with a message identifying the path and the steps attempted.
