...
- [x] 15.1 Update any unit tests that directly call `element._render()` to use `await element._render()` with `asyncio.run()` or `pytest-asyncio`
- [x] 15.2 Update any unit tests that call `generate_html()` to use `await generate_html()`
- [x] 15.3 Verify the `TestRenderer` in `webcompy/testing/` handles async `_render()` — update `TestRenderer.render()` to `async def render()` and await the component's `_render()`
- [x] 15.4 Update `create_test_asgi_app()` to await `generate_html()`

## 16. Verification

- [x] 16.1 Run lint: `uv run ruff check .`
- [x] 16.2 Run format: `uv run ruff format .`
- [x] 16.3 Run type check: `uv run pyright`
- [x] 16.4 Run unit tests: `uv run python -m pytest tests/ --tb=short --ignore=tests/e2e --ignore=tests/e2e_docs`
- [x] 16.5 Run SSG: `uv run python -m webcompy generate --config docs_app.webcompy_config` (update CLI entry to use `asyncio.run()`)
- [x] 16.6 Run E2E tests: `scripts/run-e2e-tests.sh bootstrap-static`