...
The separation between browser and server configuration is now structurally enforced by the package topology. `WebComPyAppConfig` lives in `webcompy.app` (core package, included in browser wheels). `WebComPyBuildConfig` and `WebComPyServerConfig` live in `webcompy_cli.config` (CLI package, never in browser wheels). This eliminates the need for `_BROWSER_ONLY_EXCLUDE` as a mechanism to keep server-only code out of browser wheels — the physical package boundary provides natural exclusion.
