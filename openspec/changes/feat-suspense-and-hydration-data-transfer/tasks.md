...
- [x] 1.1 Create `packages/webcompy/src/webcompy/hydration/__init__.py` with public API
      exports and the `has_resolved_data()` helper.
- [x] 1.2 Create `packages/webcompy/src/webcompy/hydration/_payload.py` with
      `TransferPayload`, `TransferFetchEntry`, `TransferAsyncResultEntry`,
      `serialize_payload()` and `deserialize_payload()`.
- [x] 1.3 Create `packages/webcompy/src/webcompy/hydration/_collect.py` with
      `collect_transfer_data()` — gathers `AsyncResult` states and `FetchPort` cache.
- [x] 1.4 Add `HYDRATION_DATA_KEY: InjectKey[dict[str, Any]]` to
      `packages/webcompy/src/webcompy/di/_keys.py` and export from
      `packages/webcompy/src/webcompy/di/__init__.py`.
- [x] 1.5 Define the payload schema as a JSON object:
      `{"__webcompy_transfer_version__": 1, "fetches": {...}, "async_results": {...}}`.
- [x] 1.6 `serialize_payload()` produces HTML-escaped JSON.
- [x] 1.7 `deserialize_payload()` returns `TransferPayload | None` — `None` on parse
      error or unknown version.

## Section 2: ServerFetchPort Response Cache

- [x] 2.1 Add `_response_cache: dict[str, Response]` to
      `packages/webcompy-server/src/webcompy_server/ports/_fetch.py:ServerFetchPort.__init__()`.
- [x] 2.2 In `ServerFetchPort.fetch()`, after a successful self-site request, cache the
      response keyed by URL (GET) or `f"{method}:{url}:{body}"` (non-GET).
- [x] 2.3 On cache hit, return the cached response without making a network request.
- [x] 2.4 Add `get_transfer_data() -> dict[str, TransferFetchEntry]` that returns
      cache contents in transfer payload format (excluding external URL responses).
- [x] 2.5 Add `clear_cache()` for cleanup between SSR renders.
- [x] 2.6 Add `is_self_site_url()` to `FetchPort` ABC in
...
- [x] 3.1 In `packages/webcompy/src/webcompy/aio/_async_result.py`, add
...
- [x] 3.2 Modify `useAsyncResult` (in `packages/webcompy/src/webcompy/components/_hooks.py`) to check
...
- [x] 3.3 Ensure the transfer payload lookup uses `inject(HYDRATION_DATA_KEY, default=None)`
      so absence of the key is handled gracefully (no transfer = normal lifecycle).

## Section 4: BrowserFetchPort Cache Population

- [x] 4.1 Add `_response_cache: dict[str, Response]` to
      `packages/webcompy/src/webcompy/ports/_browser/_fetch.py:BrowserFetchPort.__init__()`.
- [x] 4.2 Add `populate_from_transfer(data: dict[str, TransferFetchEntry])` method that
      converts transfer entries into `Response` objects and stores them.
- [x] 4.3 In `BrowserFetchPort.fetch()`, check `_response_cache` for matching URL before
      making a network request. On hit, return the cached `Response` without calling
      `browser.fetch()`.
- [x] 4.4 Add a key generation helper that matches `ServerFetchPort`'s format for
      non-GET requests (method + URL + body).

## Section 5: HTML Payload Injection

- [x] 5.1 In `packages/webcompy/src/webcompy/app/_root_component.py`, add
      `_collect_transfer_data() -> str`:
      - Uses `collect_transfer_data()` from hydration module.
      - Returns serialized payload string.
- [x] 5.2 In `packages/webcompy-server/src/webcompy_server/_html.py:generate_html()`,
      after rendering the app root:
      - Call `ctx._root._collect_transfer_data()`.
...
- [x] 5.3 If `generate_html()` is called without prerender, the script tag is omitted
      (only prerendered content gets the payload).

## Section 6: Browser Initialization Restore

- [x] 6.1 In `packages/webcompy/src/webcompy/app/_app.py:app.run()`, before the first
      render:
...
- [x] 6.2 Remove the `<script>` element from the DOM after reading to free memory.
- [x] 6.3 After all component restorations, clear the `async_results` reference to
      allow garbage collection of the payload (the DI scope reference is naturally
      cleaned up during render context lifecycle).
...
- [x] 7.1 Create `packages/webcompy/src/webcompy/elements/types/_suspense.py` with
      `SuspenseElement` class extending `DynamicElement`.
- [x] 7.2 Implement `__init__` accepting `fallback`, `children`, `error_fallback`, and
      `timeout` parameters.
- [x] 7.3 Implement `_on_set_parent()` — no signal subscriptions needed.
- [x] 7.4 Implement `_render()`:
...
- [x] 7.5 Implement `_resolve()` — replaces fallback with children using
      `_patch_children()`.
- [x] 7.6 Implement `_handle_error()` — renders `error_fallback` if provided, otherwise
      keeps fallback and logs a warning.
- [x] 7.7 Implement `_remove_element()` with proper cleanup of pending async tasks and
      any registered callbacks.
- [x] 7.8 Implement `_hydrate_node()`:
...
- [x] 9.1 Create `openspec/changes/feat-suspense-and-hydration-data-transfer/specs/suspense/spec.md`
...
- [x] 9.2 Create `openspec/changes/feat-suspense-and-hydration-data-transfer/specs/hydration-data-transfer/spec.md`
...
- [x] 9.3 Update `openspec/specs/async/spec.md` to mention `Suspense` as a
      complementary approach to `use_async_result`.
- [x] 9.4 Update `openspec/specs/elements/spec.md` to reference the `Suspense` element
      type.
- [x] 9.5 Update `openspec/specs/async-rendering/spec.md` to note the
...
- [x] 10.1 `npx @fission-ai/openspec@latest validate --specs` — all 59 specs pass.
- [x] 10.2 `npx @fission-ai/openspec@latest validate --changes` — this change passes.
- [x] 10.3 `uv run ruff check .` — no errors.
- [x] 10.4 `uv run ruff format .` — no changes.
- [x] 10.5 `uv run pyright` — 0 errors.
- [x] 10.6 `uv run python -m pytest tests/ --tb=short --ignore=tests/e2e --ignore=tests/e2e_docs`
