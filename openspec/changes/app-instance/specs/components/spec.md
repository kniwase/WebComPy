...
The framework SHALL maintain a per-app registry of component generators by name. If two components are registered with the same name within the same app, an error SHALL be raised. During the transition period, a module-level default store SHALL exist for backward compatibility with `ComponentGenerator` auto-registration at import time.
...
Each component instance SHALL be able to set the document title and meta tags through the app-scoped `HeadPropsStore`. When multiple components set the title, the most recently rendered one SHALL take precedence. When a component is destroyed, its head entries SHALL be removed from the app-scoped store.
...
- **THEN** each app SHALL have its own `HeadPropsStore`
