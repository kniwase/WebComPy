...
### D11: Dropdown trigger activation stops event propagation; outside-click listens in the capture phase
...
Because stopping propagation also hides trigger clicks of *sibling* dropdowns from bubble-phase document listeners, the Dropdown's own outside-click listener SHALL register in the **capture phase** (`DOMPort.add_document_event_listener(..., capture=True)` — additive port extension; capture listeners run before the target's own handlers and are unaffected by bubble-phase `stopPropagation`). This keeps every behavior correct at once: a click on the dropdown's own trigger is excluded by containment (no toggle race, D4), a click on another dropdown's trigger closes this menu before that trigger opens (mutual exclusivity), and host bubble-phase "close all" listeners no longer observe trigger activations. Other overlay components have no trigger element (they are opened by prop), so the trigger propagation change applies to Dropdown only. The `stopPropagation` call is guarded (`hasattr`) because fake test events may not implement it.
...
### D15: Dropdown menus are anchored to their trigger by default

The mobile-navbar verification showed the deeper gap behind the positioning regressions: the framework Dropdown teleports its menu to `body` and delegates all positioning to consumer CSS, forcing every consumer to reinvent trigger measurement (the docs app hand-rolled it on `main`, dropped it for a fixed CSS anchor in D13, and the fixed anchor then covered the expanded mobile strip — the open hamburger menu visually disappeared behind the opaque, higher-z-index menu). Positioning therefore becomes a framework capability:

- **Measurement lives in the headless layer** (behavior, per the two-layer contract): when the menu is open, the headless Dropdown queries the trigger's bounding rect through the DOM port (`getBoundingClientRect`) and re-measures on `scroll` (document, capture) and `resize` (window) via the existing `use_document_event`/`use_window_event` composables, registered only while open and removed on close/unmount (D4 listener lifecycle).
- **Offsets are supplied as inline styles on the teleported menu**: `top: {rect.bottom}px`, and horizontally `left: {rect.left}px` for `align="start"` or `right: calc(100vw - {rect.right}px)` for `align="end"`. Inline styles win over consumer CSS, which is exactly the point for the default, but would make a consumer's own positioning impossible — so the themed layer exposes a `positioning: "anchor" | "none"` prop (default `"anchor"`); `"none"` restores pure consumer-controlled positioning. `align` (`"start" | "end"`, default `"start"`) is read by the headless layer.
- **Viewport-relative math, flip/shift out of scope**: offsets use viewport units and `position: fixed` (the menu is already fixed-positioned by consumers), so scrolling the page re-measures via the scroll listener rather than needing a collision-flip engine (Tooltip/Popover remains the follow-up change for that).
- **SSR and closed menus**: the server cannot measure; menu styles fall back to no offsets (consumer CSS default) and the menu is `hidden` while closed, so the fallback is never visible. The first open always measures before/at reveal.
- **docs_app navbar**: drops its fixed-anchor CSS (`top`/`right` overrides for `.navbar-dropdown`, desktop and mobile) and the `align="end"` default for the desktop menus; the anchor follows each trigger, which restores the nested mobile-expansion look (menu opens directly below the trigger row, strip stays visible above) and keeps the desktop right-aligned menus below the navbar row.

Rationale for defaulting the themed layer to anchored positioning: the themed Dropdown ships in this same change, so there is no compatibility surface; a wrong-position menu is the kind of silent defect consumers cannot fix without reimplementing measurement, and the escape hatch keeps full control available.
...
- **Anchor offsets as inline styles (D15)**: inline `top`/`left`/`right` override consumer CSS by design; `positioning="none"` is the escape hatch. Re-measure listeners exist only while a menu is open, so closed dropdowns add no listeners. Rapid scroll/resize during an open menu re-measures per event — acceptable for v1 (no rAF throttling); flip/shift collision handling is explicitly out of scope.
