...
Provide a runtime light/dark/system theme system for WebComPy applications that is reactive, SSR-safe, and cookie-persistent. The system uses the `<html>` element's `data-theme` attribute as the single source of truth, the framework's DI scope for SSR initial-value injection, and a `Signal[Theme]` for reactive updates on the client.
