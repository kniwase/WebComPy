...
When the SSR timeout expires, pending async operations SHALL be cancelled via `asyncio.wait_for()` task cancellation. Components that have had `__init_component()` called SHALL have `_remove_element()` called on them to clean up mounted DOM nodes and dispose effect scopes. Components that never reached `__init_component()` (i.e., `_pending_async_template` is still set and no DOM nodes were mounted) SHALL be removed from their parent's children list without calling `_remove_element()`, consistent with the error handling in `feat-async-component-setup`. The fallback content SHALL then be rendered in place of the cancelled children. If nested `Suspense` boundaries exist within the cancelled subtree, their DI scopes provided by `SUSPENSE_RESOLVING_KEY` SHALL be disposed transitively — the outer Suspense's `finally` block SHALL close its own DI scope, and inner Suspense scopes that were created as children SHALL be disposed along with their component stores during `RenderContext.dispose()`.
...
If the transfer payload contains resolved `AsyncResult` data for the Suspense's children (checked via `has_resolved_data()` from the hydration module), `_hydrate_node()` SHALL call `__init_component()` **synchronously** with the cached template from the transfer payload, bypassing the `await` of `_pending_async_template`. Because the data is already available from SSR, no async resolution is needed — `_pending_async_template` SHALL be set to `None` and `__init_component()` SHALL be called directly with the resolved template. During this synchronous `__init_component()` call, lifecycle hooks (`on_before_rendering`, `on_after_rendering`) are registered as properties on the component but are NOT invoked — hooks only fire during `_render()`. After `__init_component()` returns, `_hydrate_node()` SHALL schedule `asyncio.ensure_future(self._render())` to invoke the registered hooks normally and render the children DOM nodes. This ensures hooks registered via `__init_component()` during hydration fire during the subsequent async `_render()` exactly as they would in a non-hydrated browser rendering.
