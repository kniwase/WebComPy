...
Every headless component SHALL accept a `class` prop applied to its root element, and multi-part components SHALL accept part-specific class props with documented names. User-supplied classes SHALL be appended after framework classes so that user rules take precedence at equal specificity. Class pass-through and `data-state` attributes together constitute the headless styling surface.

#### Scenario: User class reaches the root element

- **WHEN** a headless component is rendered with `class="my-custom"`
