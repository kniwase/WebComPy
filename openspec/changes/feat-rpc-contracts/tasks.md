...
- [ ] 5.2 Add typed contract usage to unit tests so pyright validates inference (result types including `batch` `tuple[R1, R2]` / `batch()->tuple[()]` and `notify()->None` and `RpcStream[T]`/`RpcSubscription[E]` element types) as part of the CI type check
