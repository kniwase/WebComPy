...
- `_run_refresh_sync` (`packages/webcompy/src/webcompy/elements/types/_dynamic.py`) gains a Pyodide-specific branch: instead of blocking on `loop.run_until_complete`, the refresh coroutine is scheduled on the event loop via `aio_run` (the same mechanism `_resolve_async_callback` already uses for async signal callbacks), wrapped so exceptions are logged via `logging.error` instead of propagating into the JS event handler.
