...
- [ ] 2.1 Unit tests with a dict-backed fake storage (`tests/test_storage_composable.py`): round-trip, missing key → default (value and factory forms), stored `null` vs missing, corrupted JSON → warning + default, non-serializable value → warning + skip, setItem failure → swallowed
- [ ] 2.2 Server-path test: no storage access in non-PyScript env (assert helper-level isolation and `Reactive(default)` return)
- [ ] 2.3 Outside-setup call emits no warning
- [ ] 2.4 Component integration via `webcompy_testing` TestRenderer: signal usable in template and updates trigger writes
