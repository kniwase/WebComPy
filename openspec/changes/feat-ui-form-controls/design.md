...
- **DI context dependency**: FormField wiring relies on child-component DI scopes resolving through the parent component's scope (verified in `components/_component.py`). If slot-rendered controls failed to resolve, wiring silently degrades to standalone behavior; a dedicated unit test pins the provide/inject path end to end.
