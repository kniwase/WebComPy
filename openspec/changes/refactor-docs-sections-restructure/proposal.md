...
- A new normative rule: cross-links in Markdown bodies SHALL use rendered-site URLs (optionally with heading anchors), never `.md` source paths.
...
- `docs-site-documents`: manifest section model gains the category URL scheme (`/documents/<category>/...`); the signal-stream URL-preservation requirement is superseded (the page moves like the others); Prev/Next scenario generalized; new requirement for body cross-link correctness.
...
- `tests/`: manifest-related tests (prev/next order, validation fixtures) updated
