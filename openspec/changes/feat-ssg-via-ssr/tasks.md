...
- [ ] 2.4 Update `packages/webcompy-testing/src/webcompy_testing/_asgi.py:create_test_asgi_app()` — since `html_generator` is now async, update the test ASGI handler to await it. Verify that `httpx.ASGITransport`-based tests still pass.
...
- [ ] 3.4 Update the CLI entry point in `packages/webcompy-cli/src/webcompy_cli/__main__.py` for the `start` command — no change needed, `run_server()` remains synchronous and passes the ASGI app to `uvicorn.run()`.
...
- [ ] 4.5 Update the CLI entry point for `generate` command in `packages/webcompy-cli/src/webcompy_cli/__main__.py` — call `asyncio.run(generate_static_site())` instead of calling it synchronously.
...
- [ ] 5.1 Update `packages/webcompy-testing/src/webcompy_testing/_asgi.py:create_test_asgi_app()` — if `create_asgi_app()` signature changed, update the test utility accordingly. The test utility should still work with `httpx.ASGITransport`.
