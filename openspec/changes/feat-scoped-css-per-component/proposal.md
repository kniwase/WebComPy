...
- Update SSG `_html.py` to generate `<style>` elements from `RenderContext.scoped_styles` dict, and render head content from `HeadElement` VDOM tree.
...
- Affected code: `webcompy/app/_root_component.py`, `webcompy/app/_render_context.py`, `webcompy/components/_generator.py`, `webcompy/cli/_html.py`, `webcompy/cli/_generate.py`, `webcompy/elements/` (new `HeadElement`)
