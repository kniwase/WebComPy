...
- *`DOMPort` interface break*: adding the abstract `create_comment` method makes custom `DOMPort` subclasses non-instantiable until they implement it. → Mitigation: the port ABC documents the new method, and the proposal Impact section records it as a breaking interface change; custom ports must implement one method mirroring `create_text_node`. A runtime fallback that degrades to a text anchor is intentionally not provided, since it would reintroduce the layout shift this change fixes.
