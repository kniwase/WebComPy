...
- **APIs**: the `DOMPort` ABC gains the required method `create_comment()`. This is a breaking interface change for custom `DOMPort` implementations: third-party subclasses must implement `create_comment` (returning a comment node, mirroring `create_text_node`) to remain instantiable. The built-in browser, server, and testing implementations are updated in this change. The `\u200b` anchor is an internal SSR serialization detail, not a public API; no other breaking change for app code.
