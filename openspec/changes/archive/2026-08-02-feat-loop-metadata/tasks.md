...
- [x] 3.2 Build per-item attribute Computeds (`index`/`index0`/`revindex`/`revindex0`/`first`/`last`/`length`) that read the source signal directly (a shared per-loop `positions` Computed was rejected: element purge destroys shared members, and the browser's deferred async refresh leaves shared state stale)
...
- [x] 6.2 Run `uv run ruff check .`, `uv run ruff format --check .`, `uv run pyright`, `uv run python -m pytest tests/ --tb=short`
- [x] 6.3 Run relevant e2e groups via `scripts/run-e2e-tests.sh` (template groups) and `uv run python -m webcompy generate` on docs_app to confirm the removed page breaks nothing
