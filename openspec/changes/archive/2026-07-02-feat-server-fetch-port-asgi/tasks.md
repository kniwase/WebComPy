- [x] **Task 1**: Add `is_self_site_url()` method to `FetchPort` ABC
...
- [x] **Task 2**: Rewrite `ServerFetchPort` with dual-client architecture
...
- [x] **Task 3**: Expose `ServerFetchPort` from `WebComPyApp` and configure base_url
...
- [x] **Task 4**: Configure `ServerFetchPort` in `create_asgi_app()`
...
- [x] **Task 5**: Configure `ServerFetchPort` in `generate_static_site()`
...
- [x] **Task 6**: Add unit tests for URL classification
...
- [x] **Task 7**: Add unit tests for `ServerFetchPort.configure()` and self-site routing
...
- [x] **Task 8**: Add integration test for base_url resolution
...
- [x] **Task 9**: Run lint, typecheck, and tests
  - `uv run ruff check .`
  - `uv run ruff format .`
  - `uv run pyright`
