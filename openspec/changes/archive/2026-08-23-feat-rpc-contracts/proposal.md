...
- **New capability `rpc-contracts`**: three contract classes defined in a shared, dependency-neutral schema module (`my_app/rpc_schema.py`) that both environments import:
  ...
  - `RpcCall[P, R]` — public awaitable returned by `Procedure`; `await` performs a single call, passing the call to `batch`/`notify` defers I/O; `bool(RpcCall)` and `len(RpcCall)` raise `TypeError` (no truth value).
  - `batch(*calls: RpcCall, return_exceptions=False)` — typed batch over HTTP and WebSocket (free function, `from webcompy.rpc import batch`). 0 calls is a no-op returning `()` with no I/O; 1..n calls share one transport round-trip (HTTP: one POST array, WebSocket: one array frame) reusing the existing `dispatch_payload` batch path; heterogeneous batches infer `tuple[R1, ..., Rn]` via `() -> tuple[()]` + 1..6 overloads (gather-style, variadic fallback is `tuple[R, ...]`), `return_exceptions=True` surfaces per-call `RpcError`s as `R | RpcError` entries in input order.
  - `notify(*calls: RpcCall)` — typed fire-and-forget notification over HTTP and WebSocket (free function, `from webcompy.rpc import notify`). Takes `RpcCall`s produced by `Procedure` (same `RpcCall` gate as `batch`), 0 calls is a no-op returning `None` with no I/O; 1..n calls share one transport round-trip as an id-less array (HTTP: one POST array without `id`, WebSocket: one array frame without `id`) reusing the existing batch wire; `204` / no-frame response. `batch` and `notify` apply only to `Procedure` calls; streaming and subscription contracts are rejected.
- `RpcTransport` protocol (`call` / `notify` / `stream` / `subscribe`) is the official low-level transport contract, implemented by `RpcWsClient` (all four) and a new `RpcHttpClient` (call/notify/stream; subscribe raises `RpcError`), which also makes contracts usable during SSR (HTTP bake preserved, including `batch` via the array POST bake; `notify` is not baked due to `204`). Public callers use `Procedure`/`batch`/`notify` with `RpcCall`, not `RpcTransport` directly.
...
- pyright then statically checks contract usage: nonexistent contract names are undefined-name errors, `AddParams("x")` is a type error, `await add(...)` is inferred as `int`, `await batch(add(client,p1), get_user(client,p2))` is inferred as `tuple[int, User]`, `await batch()` is `tuple[()]`, `await notify(add(client,p1))` is `None`, and `async for` over a subscription is inferred as the event type.
- The JSON-RPC 2.0 wire (HTTP POST + WebSocket frames, batch, notifications, error codes, allowlist decoding, transfer `meta`) and the subscription/stream wire protocols (cursors, replay, rejoin, resync, SSE, `stream_id`) are unchanged; `batch` and `notify` reuse the existing batch wire (`notify` as id-less array).
...
- **Code**: new `webcompy/rpc/_contracts.py` (contract classes + `RpcCall` (`__bool__`/`__len__` raise `TypeError`, single-await) + `RpcTransport` + `RpcHttpClient` + `batch` (`() -> tuple[()]` + 1..6 overloads + `return_exceptions`, HTTP/WS dispatch) + `notify(*RpcCall)` (variadic, empty no-op, id-less array dispatch)); `webcompy/rpc/_registry.py` (bind + validation including `Subscription(replay_size)`, remove `register`/`register_subscription`); `webcompy/rpc/_client.py` (internalized module functions); `webcompy/rpc/_ws_client.py` (transport re-designation + WS batch/notify array-frame handling, no other wire changes); `webcompy/rpc/__init__.py` exports; `webcompy_cli/_server.py` wording only. Server dispatcher/endpoints unchanged (batch wire already supported).
...
- No changes to the JSON-RPC 2.0 wire protocol, transfer `meta` codec, subscription rejoin/replay/resync semantics, or the `RpcStream` / `RpcSubscription` handle objects (batch/notify reuse the existing batch wire; notify as id-less array).
- No `use_rpc` composable or DI-injected default transport — transports are passed explicitly to contracts (and to `RpcCall` via the originating `Procedure` call); public callers use `batch`/`notify` free functions with `RpcCall`, not `RpcTransport` directly.
