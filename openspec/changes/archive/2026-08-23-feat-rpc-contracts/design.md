...
- `feat-sse-post` (`0d65127e`) and `feat-rpc-streaming` (`7e029e69`) are implemented and merged into the base branch (rebase completed at `ded3285f`; validation confirmed no title drift; re-integrated onto `52ebe5e8` — hydration payload lifecycle fix — with no conflicts; re-integrated onto `a0be6597` — startup profiler restoration — with no conflicts).
