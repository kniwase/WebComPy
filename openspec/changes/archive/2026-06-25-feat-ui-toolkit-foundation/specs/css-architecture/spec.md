...
Define the framework-provided CSS architecture: the layer order, the role of each static CSS file, and the automatic `@layer webcompy-scope` wrapping of component `scoped_style` rules. This gives applications a predictable cascade and lets framework CSS and component CSS coexist without specificity wars.
