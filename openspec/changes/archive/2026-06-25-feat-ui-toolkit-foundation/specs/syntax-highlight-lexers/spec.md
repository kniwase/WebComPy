...
Define the extension point for syntax-highlighting lexers in WebComPy. The framework provides built-in lexers for Python, Bash, and TOML and a registry mechanism for users to add their own lexers. The Pygments adapter is included as a skeleton to enable future adoption without API changes.
