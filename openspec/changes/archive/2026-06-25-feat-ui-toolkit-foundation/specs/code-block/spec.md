...
Provide a `CodeBlock` component that renders syntax-highlighted source code at render time (no client-side JavaScript) using a pluggable `Lexer` protocol. The component supports both static code (highlighted once at render time, baked into the SSR/SSG HTML) and dynamic code (highlighted reactively as a `Signal[str]` changes), and emits HTML that is both self-styling via design tokens and compatible with Pygments short class names.
