...
The CLI SHALL discover the `WebComPyApp` instance by first checking the `--app` CLI flag, then falling back to `webcompy_config.py`'s `app_import_path`. The `app_import_path` SHALL follow the `"module.path:variable_name"` format used by `get_app_from_import_path()`.
