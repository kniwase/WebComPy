...
The child SHALL be a single real-DOM node: an `ElementBase` instance (including `Component` roots). `DynamicElement` children (`Fragment`, `Teleport`, `switch`, `repeat`, `Suspense`, ...), text nodes, and other shapes are rejected with a framework validation error (`WebComPyException`), because the class protocol targets one concrete DOM node and multi-node/anchor shapes have no defined class target. When the generator result changes between two elements with the same tag name, the node is patched in place without a sequence (mirroring `_patch_children` matching); only a shape change (different tag) triggers the sequential leave-then-enter replacement. After patching, the patched element calls `_render()` unconditionally (the same-tag patch branch originally gated it on `_mounted is None`, but `_patch_children` adopts the top-level node and sets `_mounted = True`, which skipped mounting unmatched new descendants and skipped component lifecycle hooks — mirroring `SwitchElement._refresh`, the patch is followed by an unconditional render that is a no-op for adopted nodes and mounts new inner children). Signal-driven renders (patch and mount branches) wrap the child `_render()` in the same `start_defer_after_rendering`/`end_defer_after_rendering` pair as `SwitchElement._refresh`, scheduling collected `on_after_rendering` hooks as host-port macro tasks so hooks observe post-reindex state rather than firing synchronously mid-dispatch.
