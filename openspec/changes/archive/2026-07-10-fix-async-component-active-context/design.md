...
1. **Phase 1 (`__setup()`)**: The setup function `component_def(context)` is called. For async definitions, this creates a coroutine object but does NOT execute the body. `_active_component_context` and `_active_scope` are set and immediately reset within `__setup()`.

2. **Phase 2 (`_render()`)**: The stored coroutine is awaited, executing the async body. At this point, `_active_component_context` and `_active_scope` have already been reset to `None`.
...
**Goals:**
- Make `_active_component_context` and `_active_scope` available during async setup body execution
...
### Decision 2: `ComponentRenderState` bundles Context, EffectScope, and cleanup

**Choice**: A dataclass holds the `Context`, `EffectScope`, and a `framework_cleanup` callable, stored on `self._render_state` during `__setup()`.

**Rationale**: These two objects must always be activated together. Bundling them prevents partial activation (e.g., activating context but forgetting scope). The `component_context()` manager takes a `ComponentRenderState` and activates both. The `framework_cleanup` callable centralizes DI scope and EffectScope disposal so both `_remove_element()` and `_detach_from_node()` invoke the same cleanup path.
...
**Implementation**: The re-extraction logic is idempotent — for sync components (where hooks were already extracted in `__setup()`), re-extraction yields the same result. For async components, re-extraction captures hooks/results registered during body execution. Each `Component` tracks whether setup results have been extracted via `_async_setup_extracted`; `_render()` invokes `_refresh_async_setup_results()` whenever this flag is `False`.

**Suspense path**: When `SuspenseElement._render()` resolves coroutines, it must wrap each coroutine with `component_context()` so the body executes with context active. After `asyncio.gather()` completes, `SuspenseElement._resolve_component_templates()` calls `Component._refresh_async_setup_results()` so hooks and async results are populated before the resolved children render. The `_async_setup_extracted` flag makes the refresh robust regardless of whether rendering occurs inside or outside the temporary Suspense DI scope.
...
- **[Re-extraction timing for Suspense path]** In the Suspense path, the temporary `SUSPENSE_RESOLVING_KEY` DI scope may be exited before `Component._render()` runs, so relying on that scope to detect re-extraction is fragile. → Mitigation: `_refresh_async_setup_results()` is invoked directly in `SuspenseElement._resolve_component_templates()`, and `_async_setup_extracted` ensures `Component._render()` performs the refresh if any path misses it.
