...
- **AND** `webcompy_config.py` exists at the project root with `from webcompy_cli.config import WebComPyBuildConfig` and `config = WebComPyBuildConfig(app, ...)`
...
Every generated HTML page SHALL include PyScript bootstrapping. The bootstrap import SHALL use `from {package}.app import app` (not `from {package}.bootstrap import app`). The mount div SHALL use the ID from `WebComPyAppConfig.selector` (without the `#` prefix). The bootstrap format itself SHALL NOT change.
...
CLI flags SHALL override values from `WebComPyBuildConfig`. The following flags SHALL be supported: `--dev`, `--port`, `--dist`, `--config`, `--serve-all-deps`, `--no-serve-all-deps`, `--wasm-serving`, `--runtime-serving`, `--standalone`, `--no-standalone`, `--wheel-mode`. Behavior for all flags SHALL remain unchanged; only the package where `WebComPyBuildConfig` is defined has moved to `webcompy_cli.config`.
...
- **THEN** the server SHALL build a single wheel containing `webcompy` core, app code, and ALL pure-Python dependencies
...
- **WHEN** a developer runs `python -m webcompy start --dev --no-serve-all-deps`
- **THEN** the server SHALL build a single wheel containing `webcompy` core, app code, and locally-bundled dependencies only
...
- **AND** the framework wheel SHALL contain `webcompy` core only
