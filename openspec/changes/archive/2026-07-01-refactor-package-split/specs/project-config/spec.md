...
`python -m webcompy init` SHALL generate a single `webcompy_config.py` at the project root with `from webcompy_cli.config import WebComPyBuildConfig` and `config = WebComPyBuildConfig(...)`, and an `app.py` file (not `bootstrap.py`).

#### Scenario: Running webcompy init
- **WHEN** a developer runs `python -m webcompy init`
- **THEN** `webcompy_config.py` SHALL be created at the project root with `from webcompy_cli.config import WebComPyBuildConfig` and `config = WebComPyBuildConfig(app, ...)`
