...
- **AND** `create_asgi_app(app)` from `webcompy_cli` SHALL return a mountable ASGI application
- **AND** `generate_static_site(app)` from `webcompy_cli` SHALL produce static HTML with head content from `HeadElement`
