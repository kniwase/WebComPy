# Reactive Scoped Style

## Purpose

Per-component CSS in WebComPy is currently defined via a static dictionary assigned to the component class after the decorator runs (`MyComponent.scoped_style = {...}`). The values are plain strings, so they cannot react to runtime state.

This capability introduces a **reactive scoped style** that can close over signals defined inside the component setup function, evaluate as a `Computed`, and update the corresponding `<style>` element in the document head on every change. Static `scoped_style` continues to work unchanged; the two systems coexist.
