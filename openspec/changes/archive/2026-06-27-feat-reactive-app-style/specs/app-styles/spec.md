...
WebComPy applications may need to inject global CSS rules whose content depends on runtime state (signals, computed values, or other reactive primitives). The existing per-component `scoped_style` and `data-*` attribute mechanisms are insufficient for app-level concerns like dynamic design-token overrides, runtime theming variables, or per-user customizations that affect many components at once.

This capability introduces a single primitive — `app.append_style(content)` — that accepts a plain string or a `Computed[str]` and renders it into a per-registration `<style data-webcompy-dynamic="{id}">` element inside `<head>`. Reactive content updates the element's `textContent` on every change.
