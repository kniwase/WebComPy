...
When `standalone=True` is set, `webcompy-lock.json` SHALL include a `standalone_assets` section recording the URLs and SHA256 hashes of downloaded PyScript and Pyodide runtime files. This section coordinates with `feat-pyscript-local-serving`, `feat-deps-local-serving`, and `feat-wasm-local-serving`.
