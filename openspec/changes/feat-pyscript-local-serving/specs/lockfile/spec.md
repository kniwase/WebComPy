...
When `standalone=True` is set, `webcompy-lock.json` SHALL include a `standalone_assets` section recording the URLs and SHA256 hashes of downloaded PyScript and Pyodide runtime files (not dependency packages — those are handled by `feat-deps-local-serving` and `feat-wasm-local-serving`).
