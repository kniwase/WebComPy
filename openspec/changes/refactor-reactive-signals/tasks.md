...
- [x] 2.5 Implement `on_after_updating(func)` and `on_before_updating(func)` as factory methods that create `CallbackConsumerNode` instances, register them as live consumers of the producer node via `producer_add_live_consumer()`, and return a `callback_id` for cleanup. Maintain backward compatibility with `ReactiveStore.remove_callback()` by delegating to `consumer_destroy()`
- [x] 2.6 Convert `ReactiveStore` into a thin backward-compatible shell that delegates `add_reactive_instance`, `add_on_after_updating`, `add_on_before_updating`, `remove_callback`, `callback_after_updating`, `callback_before_updating`, and `detect_dependency` to the new graph infrastructure. Mark the class as deprecated
- [x] 2.7 Write/update tests for `Reactive`: equality skip (same value, equal value), version increment on change, `on_after_updating` still fires on actual change, `on_before_updating` fires before change, `ReactiveStore.remove_callback()` still works
...
- [x] 7.3 Mark `ReactiveStore` methods as deprecated in docstrings. Add `DeprecationWarning` on `ReactiveStore` direct instantiation (if any external code does this). Ensure all internal usage goes through the graph API
