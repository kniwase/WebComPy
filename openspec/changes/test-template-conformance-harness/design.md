...
Each example becomes `test_gfm_spec[NNN-section]`. Failing examples are recorded in a checked-in JSON file (`tests/conformance/xfail.json`) containing the pinned spec revision, the SHA-256 of the spec, the baseline conformance counts (passing/xfailing/total), the generation date, and the list of failing example numbers. Each example listed there is decorated with `strict=True` xfail — so an example that *starts* passing fails the suite until removed from the list. The JSON file is the single source of truth; it ties the conformance baseline to the pinned spec revision, so a revision bump that leaves the xfail list stale is detected by the spec-revision pin validation test.
...
- **Why JSON rather than plain text**: the xfail list is now coupled to the spec revision pin (the harness records the same `SPEC_REVISION` constant that the JSON references). Co-locating the pin, baseline counts, and example numbers in one file makes the cross-revision invariant testable — `test_xfail_spec_revision_matches_pin` and `test_xfail_spec_sha256_matches_pin` enforce the relationship.
- **Why `passing + xfailing == total` as a baseline invariant**: the JSON is regenerated by parsing each example; arithmetic consistency catches off-by-one errors and partial regeneration.
- A summary test (`test_gfm_conformance_rate`) reports the current pass/total count for visibility without failing; the JSON baseline is the snapshot used for cross-revision audits.
