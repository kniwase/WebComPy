...
2. HTML parser environment parity verdict: **divergence found, pin applied.** Pyodide (cpython 3.13 stdlib shipped with Pyodide 0.29.3) does not have the recent RCDATA security backport, so `<textarea><b>x</b></textarea>` was being parsed as `textarea > b > text("x")` in the browser versus `textarea > text("<b>x</b>")` on server (CPython 3.12.3 / CSP build with the backport). Fix: `TemplateTreeBuilder` now overrides `RCDATA_CONTENT_ELEMENTS = ("textarea", "title")` on the HTMLParser subclass and explicitly calls `self.set_cdata_mode(tag_lower)` inside `handle_starttag` for those tags. This is a no-op on stdlib versions that already handle RCDATA (the `set_cdata_mode` call is idempotent and lands in the same end state) and restores the RCDATA behavior on versions that lack it. The E2E scenario (`test_html_parser_environment_parity`) now passes in both prod and static modes and serves as a permanent regression guard.
3. Whether `<plaintext>` should join the rejected-tags list independent of parity results — deferred; current behavior is merely documented as a limitation.
