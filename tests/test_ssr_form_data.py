...
async def submit(request: Request):
    _received["content_type"] = request.headers.get("content-type", "")
    _received["body"] = await request.body()
...
class TestSsrFormDataIntegration:
    ...
    async def test_form_data_reaches_self_site_as_multipart_during_ssr(self):
        ...
        try:
            ...
            assert _received["content_type"].startswith("multipart/form-data; boundary=")
            boundary = _received["content_type"].rsplit("=", 1)[1]
            expected = (
                f'--{boundary}\r\nContent-Disposition: form-data; name="name"\r\n\r\nvalue\r\n'.encode()
                + f"--{boundary}--\r\n".encode()
            )
            assert _received["body"] == expected
        finally:
            await port.close()
