...
def test_app_loads(app_page, assert_no_console_errors):
    expect(app_page.locator("#webcompy-app")).to_be_visible()


def test_loading_screen_removed(app_page, assert_no_console_errors):
    loading = app_page.locator("#webcompy-loading")
    expect(loading).to_have_count(0)


def test_home_page_rendered(app_page, assert_no_console_errors):
...
def test_page_title(app_page, assert_no_console_errors):
