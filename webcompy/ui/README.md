...
- `webcompy.ui._styles` — static CSS files (`tokens.css`, `tokens-dark.css`, `reset.css`, `components.css`, `code-block.css`, `syntax-theme.css`, `index.css`). The framework auto-injects `/_webcompy-ui/index.css` into the page head.
