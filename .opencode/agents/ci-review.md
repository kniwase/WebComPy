...
**Markdown Port & Default Parser**: `MarkdownPort` ABC (`webcompy.ports._markdown`) exposes a single `render(source: str) -> str` and is provided in both `BrowserRenderContext` and `ServerRenderContext` via `MARKDOWN_PORT_KEY = InjectKey[MarkdownPort]("webcompy-port-markdown")`. `DefaultMarkdownParser` MUST be stdlib-only and SHALL apply `textwrap.dedent` before parsing, normalize tabs to two spaces for list indentation, escape fenced-code and inline-code content with `html.escape`, and apply inline transformations in the order inline code → images → links → bold → italic → strikethrough (code-before-formatting protects `` `*not bold*` `` content from being mangled by bold/italic/link regexes; images-before-links prevents `![]()` from being partially consumed by the link regex; bold-before-italic prevents `**` from being tokenized as italic). Template syntax `{{ }}` and `{% %}` MUST pass through inline processing unchanged. Lines starting with `<` SHALL be detected as HTML blocks (multi-line: subsequent non-blank lines are collected until a closing `</tag>` or self-closing `/>` is found — the `/>` terminates the block ONLY when the line does not itself start with a new `<` tag, so nested self-closing elements are preserved) and emitted verbatim, preserving component tags such as `<user-card>` for later `render_template` resolution. The parser's conformance target is GFM (CommonMark + tables, task list items, strikethrough, autolinks, disallowed raw HTML); current deviations are tracked in `tests/conformance/xfail.txt` and `tests/test_markdown_parser.py` (via the `gfm_deviation` marker) and are scheduled for removal by the parser-rewrite follow-up changes. **Code-block template protection**: Fenced code blocks and inline code spans MUST escape `{` to a non-printable placeholder (`\x00wc-lb\x00`); the placeholder is restored to literal `{` at bind time (`bind_text_part` / `resolve_attr._render_parts`). NEVER restore at parse time — `_split_directives_in_children` runs after parse and would re-interpret the restored `{% %}` as a directive, breaking the protection. **Inline tokenization**: Token keys MUST be NUL-prefixed with a per-render UUID nonce (e.g. `\x00WC{nonce}{index}\x00`) so user input cannot spoof them; restore loop MUST be convergent (repeat until no change) so `*a **b** c*` and `~~a **b** c~~` nest correctly regardless of the regex order. **URL allow-list**: `DefaultMarkdownParser` MUST validate link/image URLs against an allow-list of schemes: `http:`, `https:`, `mailto:`, relative URLs (no scheme), and fragment identifiers (`#...`). Disallowed schemes (notably `javascript:`, `data:`, `vbscript:`) MUST NOT produce a link or image element; the link text (or image alt) SHALL be rendered as plain text. **Lists**: `_LIST_RE` MUST recognize `-`, `*`, and `+` as unordered markers; multi-line list-item text MUST be joined with a single space (continuation lines are prefixed with `" "` before joining); ordered lists with a first-marker number other than 1 MUST emit `<ol start="N">`; spaced HR patterns `* * *`, `- - -`, `_ _ _` MUST be recognized as `<hr>` before list/paragraph handling.

**GFM Conformance Harness**: `tests/conformance/_spec_examples.py` declares the conformance target as a pinned cmark-gfm commit URL and SHA-256 — the spec.txt content is NEVER committed (CC BY-SA 4.0 licensing). On first use, `ensure_spec_file()` fetches the spec via stdlib `urllib.request`, hash-verifies, and caches at `tests/conformance/.tmp/gfm_spec.txt` (gitignored via `**/.tmp/`). `WEBCOMPY_GFM_SPEC_TXT` env var overwrites the cache path for air-gapped environments. Cache missing AND network unavailable → fail with clear instructions (NOT silent skip). `extract_examples()` parses the 32-backtick `example` / `example <info>` fences and `\n.\n` markdown/HTML separator; `<info>` (e.g. `example table`) is supported because GFM extension fences carry an info string. Tab marker `→` (U+2192) is converted to `\t` in both segments. The parametrized suite (`test_gfm_spec.py`) compares `DefaultMarkdownParser().render(md).rstrip()` against `expected_html.rstrip()` — `rstrip()` is the ONLY allowed normalization; internal-newline or HTML-attribute normalization MUST NOT be added (the harness measures, the parser fixes). Failing examples are tracked by example number in `tests/conformance/xfail.txt` with strict xfail (`pytest.mark.xfail(strict=True)`); an example that begins passing while still listed fails the suite — strict xfail is the conformance-improvement review hook. The `test_markdown_parser.py` deviations marked `pytest.mark.gfm_deviation` are the unit-test retirement queue for the parser-rewrite changes.

**Markdown Pipeline (`render_markdown`)**: `render_markdown(source: str, context: Mapping[str, Any] | None = None) -> ElementAbstract` accepts `str` only (no `Path` — file-based Markdown is composed by callers as `render_markdown(await load_text(path), ctx)`). The pipeline SHALL: (1) `inject(MARKDOWN_PORT_KEY).render(source)`; (2) strip `<p>` wrappers around lone `{% %}` directives using the design regex `r'<p>\s*(\{%\s*(?:if|elif|else|endif|for|endfor)\b[^%]*?%\})\s*</p>'` (the body MUST be exactly one recognized directive with a keyword from the allow-list and nothing else, so paragraphs containing directive-with-text are preserved verbatim); (3) call `_render_nodes(html, context)` (the Change 1 shared pipeline that bypasses single-root validation). After `_render_nodes`, the implementation MUST drop `None` nodes and whitespace-only string nodes before counting/wrapping, because the underlying `split_text` preserves inter-element newlines as raw `"\n"` strings; otherwise FragmentElement would gain junk `TextElement` children and the single-root shortcut would misfire. Zero or multiple surviving elements return `FragmentElement(elements)`; exactly one returns the element directly. A `FragmentElement` is NOT a valid component root (Component root requires `isinstance(node, Element)`) — developers MUST wrap multi-root Markdown in an explicit `Element` (`html.ARTICLE({}, render_markdown(...))`).
