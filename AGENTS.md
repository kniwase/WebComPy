...
| `ui-overlay` | First-party overlay components (Modal, Drawer, Dropdown, Toast) as headless/themed pairs: dialog accessibility contract, menu button keyboard model with propagation-owned toggle and capture-phase outside close, per-instance hydration-stable DOM ids, optional `render_closed` static menu mode, and the toast queue with live region and auto-dismiss |
