...
def generate_lockfile(
    ...
    from webcompy_cli._dependency_resolver import PackageKind, classify_dependencies
    ...
    if runtime_serving == "local":
        from webcompy_cli._pyodide_lock import PYODIDE_RUNTIME_URL_TEMPLATE
        from webcompy_cli._runtime_downloader import PYODIDE_RUNTIME_ASSETS, PYSCRIPT_OFFLINE_URL_TEMPLATE
...
def validate_local_environment(
    ...
    errors: list[str] = []
    warnings: list[str] = []
    from webcompy_cli._dependency_resolver import _find_package_dir, _get_package_version
...
def get_bundled_deps(
    ...
    if lockfile is None:
        return []
    from webcompy_cli._dependency_resolver import _find_package_dir
...
def verify_and_update_runtime_assets(
    ...
    from webcompy_cli._pyodide_lock import PYODIDE_RUNTIME_URL_TEMPLATE
    from webcompy_cli._runtime_downloader import PYSCRIPT_OFFLINE_URL_TEMPLATE, RuntimeDownloadError
