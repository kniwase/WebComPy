...
def FormField(context: ComponentContext[FormFieldProps]) -> Any:
    ...
    The field's association ids (generated from the instance's
    hydration-stable transfer id) are provided through the component DI
    scope, so bound controls rendered in the default slot adopt them: the
    native control receives the ``id`` referenced by the caption's
    ``<label for>``, and, while the field is touched and invalid,
    ``aria-invalid="true"`` plus an ``aria-describedby`` link to the
    error region. Error messages render only in the touched-invalid
    state, so an invalid-but-untouched form shows nothing on load. Group
    controls (``RadioGroup``) self-label with a ``legend``; pass no
    ``label`` here for those. The root carries ``data-state`` following
    the same gating. A standalone ``field`` prop is required; raw value
    mode controls are used without a FormField.
    ...
    children.append(
        create_element(
            "div",
            {"id": error_id, "role": "alert", "class": join_classes(_ERROR_CLASS, props.get("class_error", ""))},
