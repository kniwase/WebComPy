...
@define_component("websocket-page")
async def WebsocketPage(context: ComponentContext[RouterContext]):
    doc = await load_markdown_document("documents/websocket.md")
    context.set_title(f"{doc.metadata['title']} - WebComPy Docs")
    return docs_page_template(doc, context.props.path)


WebsocketPage.scoped_style = DOCS_PAGE_SCOPED_STYLE
