...
@define_component("rpc-contracts-page")
async def RpcContractsPage(context: ComponentContext[RouterContext]):
    doc = await load_markdown_document("documents/rpc_contracts.md")
    context.set_title(f"{doc.metadata['title']} - WebComPy Docs")
    return docs_page_template(doc, context.props.path)


RpcContractsPage.scoped_style = DOCS_PAGE_SCOPED_STYLE
