...
@define_component("rpc-websocket-page")
async def RpcWebsocketPage(context: ComponentContext[RouterContext]):
    doc = await load_markdown_document("documents/rpc_websocket.md")
    context.set_title(f"{doc.metadata['title']} - WebComPy Docs")
    return docs_page_template(doc, context.props.path)


RpcWebsocketPage.scoped_style = DOCS_PAGE_SCOPED_STYLE
