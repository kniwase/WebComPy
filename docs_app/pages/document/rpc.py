...
@define_component("rpc-page")
async def RpcPage(context: ComponentContext[RouterContext]):
    doc = await load_markdown_document("documents/rpc.md")
    context.set_title(f"{doc.metadata['title']} - WebComPy Docs")
    return docs_page_template(doc, context.props.path)


RpcPage.scoped_style = DOCS_PAGE_SCOPED_STYLE
