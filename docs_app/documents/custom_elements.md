...
description: Every WebComPy component is a Light DOM custom element with a mandatory name, layout-transparent wrapper, and universal lifecycle hooks.
---
...
The custom element name **must match the Python function name** through case conversion:
...
`define_component` validates this at definition time and raises `WebComPyComponentException` on a mismatch. Consequences:

- The name must be lowercase and contain a hyphen, so **single-word component names are invalid**: `App` cannot be defined as `@define_component("app")`. Rename to a multi-word name such as `TodoApp` (`todo-app`).
- Acronym-style names must be normalized: `HTTPRequest` does not round-trip (`http-request` resolves back to `HttpRequest`), so use `HttpRequest`.
- The template engine resolves `<user-card>` tags through the same conversion, so Python API, template tags, and DOM tags always agree.
...
Older WebComPy code used a bare `@define_component` form whose component node was the template root element. That form no longer exists; migrate every component by adding its kebab-case name:
...
- **Single-word and acronym names** must be renamed (see [Naming rules](#naming-rules)).
